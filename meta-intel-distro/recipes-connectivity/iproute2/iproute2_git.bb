...
IPROUTE2_GIT_URI ?= "https://git.kernel.org/pub/scm/network/iproute2/iproute2.git"
IPROUTE2_GIT_BRANCH ?= "main"
IPROUTE2_GIT_SRCREV ?= "${AUTOREV}"
IPROUTE2_EXTRA_PATH ?= ""
IPROUTE2_VERSION_EXTENSION ?= ""

IPROUTE2_GIT_URI_CLEANUP = "${@d.getVar('IPROUTE2_GIT_URI').replace('https://','').replace('git://','')}"
IPROUTE2_GIT_PROTOCOL = "${@'file' if d.getVar('IPROUTE2_GIT_URI').startswith('/') else 'https'}"

FILESEXTRAPATHS:prepend := "${IPROUTE2_EXTRA_PATH}:"

SRC_URI = "git://${IPROUTE2_GIT_URI_CLEANUP};protocol=${IPROUTE2_GIT_PROTOCOL};branch=${IPROUTE2_GIT_BRANCH} \
           "

SRCREV = "${IPROUTE2_GIT_SRCREV}"

PV = "git${IPROUTE2_VERSION_EXTENSION}${@'+localpatches' if d.getVar('IPROUTE2_EXTRA_PATH') else ''}"
...
# Add in SRC_URI patches from external path (IPROUTE2_EXTRA_PATH) if exists
python __anonymous() {
    import os, bb

    dir = d.getVar('IPROUTE2_EXTRA_PATH', True)
